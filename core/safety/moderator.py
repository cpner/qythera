...
JAILBREAK_PATTERNS = [
    ...
    r"pretend\s+(you|that)\s+(have|there)\s+(are|is)\s+no\s+(rules|restrictions)",
    r"bypass\s+(all|any|your)\s+(safety|content|filter)",
    r"d?\n?\s*mode",
    r"act\s+as\s+(a\s+)?(evil|dark|unethical|villain)",
...
class SafetyModerator:
    ...
    def __init__(self, toxicity_threshold=0.5, enable_jailbreak=True, enable_pii=True):
        self.tox_thresh = toxicity_threshold
        self.enable_jb = enable_jailbreak
        self.enable_pii = enable_pii
        self.tox_patterns = [re.compile(p, re.I) for p in TOXIC_PATTERNS]
        self.jb_patterns = [re.compile(p, re.I) for p in JAILBREAK_PATTERNS]
