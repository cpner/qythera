...
class VaelonDecoderLayer(Module):
    ...
    def forward(self, x, mask=None, kv_cache=None, position=0):
        ...
        if isinstance(self.ffn, MoELayer):
            ...
            ffn_out = self.ffn(h)
            aux_loss = 0.0
        return x + ffn_out, new_cache, aux_loss


class LayerList(Module):
    """Helper to track a list of layers as modules."""
...
class VaelonModel(Module):
    def __init__(self, config=None):
        ...
        self.layers = LayerList([VaelonDecoderLayer(config) for _ in range(config.num_layers)])
    ...
    def forward(self, ids, labels=None, mask=None):
        ...
        if labels is not None:
            ...
            shift_logits = Tensor(logits.data[:, :-1, :].copy())
            shift_labels = Tensor(labels.data[:, 1:].copy())
            loss = self.cross_entropy(shift_logits, shift_labels)
            loss = loss + Tensor(np.array(total_aux / max(len(self.layers), 1)))

        return logits, loss, total_aux / max(len(self.layers), 1)

    def cross_entropy(self, logits, targets):
        B, L, V = logits.data.shape
        logits_flat = logits.reshape(-1, V).data
        targets_flat = targets.reshape(-1).data.astype(int)

        max_vals = logits_flat.max(axis=-1, keepdims=True)
        exp_logits = np.exp(logits_flat - max_vals)
        probs = exp_logits / (exp_logits.sum(axis=-1, keepdims=True) + 1e-8)

        valid_mask = (targets_flat != -100).astype(np.float32)
        safe_targets = np.clip(targets_flat, 0, V - 1)
        target_probs = probs[np.arange(len(safe_targets)), safe_targets]
    ...
    def generate_ids(model, prompt_ids, max_new=512, temp=0.7, top_k=50, top_p=0.9):
        ...
        for step in range(max_new):
            ...
            if top_p < 1.0:
                ...
                sorted_logits = next_logits[sorted_idx].copy()
