...
class TestVaelonModel:
    ...
    def test_forward_with_labels(self):
        ...
        logits, loss, aux = model(ids, labels)
    ...
    def test_causal_mask(self):
        ...
        assert mask.data[0, 0, 0, 1] < 0  # masked
        assert mask.data[0, 0, 0, 0] == 0  # not masked

    def test_parameter_count(self):
        for name, cfg_func in [("small", VaelonConfig.small), ("medium", VaelonConfig.medium)]:
            model = VaelonModel(cfg_func())
            params = sum(p.data.size for p in model.parameters())
            assert params > 1000, f"{name} model too small"
