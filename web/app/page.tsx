...
export default function Home() {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      const content = d.choices?.[0]?.message?.content || 'No response received.';
      setMsgs(p => [...p, { id: (Date.now()+1).toString(), role: 'assistant', content, time: fmtTime() }]);
    } catch {
      setMsgs(p => [...p, {
        id: (Date.now()+1).toString(), role: 'assistant',
        content: 'Server offline. Start it with:\n\npython -m core.inference.server',
        time: fmtTime()
      }]);
    }
    setLoading(false);
    ...
    <div className="flex h-full" style={{height: '100dvh'}}>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0">
        ...
        <div className="glass-strong flex items-center gap-3 px-3 py-2.5 m-2 mb-0 shrink-0">
          <button onClick={() => setSidebar(!sidebar)} className="p-2 rounded-xl hover:bg-white/5 transition-colors">
        ...
        {/* Input */}
        <div className="px-3 sm:px-4 pb-3 input-area shrink-0">
          <div className="input-glass flex items-end gap-2">
            ...
            <button onClick={send} disabled={!input.trim()||loading}
              className="p-2.5 rounded-2xl bg-gradient-to-br from-violet-500 to-blue-500 text-white disabled:opacity-20 disabled:cursor-not-allowed transition-all hover:shadow-lg hover:shadow-violet-500/20 active:scale-95 shrink-0">
